...
describe('/funcionarios', () => {
    test('POST /funcionarios', async () => {
        ...
        expect(response.body.email).toMatch('funcionario@email.com');
    ...
    test('POST /funcionarios/login espera 200', async () => {
    ...
    test('POST /funcionarios/login espera 401', async () => {
        const response = await request.post(`${url}/login`).send({})
        expect(response.status).toBe(401);
        expect(response.body.msg).toMatch('Credenciais inválidas');
    });
    test('POST /funcionarios/renovar espera 200', async () => {
    ...
    test('POST /funcionarios/renovar espera 401', async () => {
    ...
    test('GET / id deve retornar 200', async () => {
        ...
        expect(response.status).toBe(200);
        expect(response.body._id).toBeDefined();
        expect(response.body.email).toBe("funcionario@email.com");
    });
    ...
    test('GET / id deve retornar 404', async () => {
        ...
        expect(response.body.msg).toBe("Funcionario nao encontrado");
    });

    test('PUT / id deve retornar 200', async () => {
        const response = await request.put(`${url}/${id}`).set("authorization", `Bearer ${token}`).send({
            email: "funcionario@email.com",
            senha: "abcd1234"
        });
        expect(response.status).toBe(200);
    ...
    test('PUT / id deve retornar 422', async () => {
        ...
        expect(response.status).toBe(422);
        expect(response.body.msg).toBe("email do funcionario é obrigatorio")
    });
